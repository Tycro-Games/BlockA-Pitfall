...
void Buffer::CopyToDevice2(bool blocking, cl_event* eventToSet, const size_t s)
{
	cl_int error;
	CHECKCL(
		error = clEnqueueWriteBuffer( Kernel::GetQueue2(), deviceBuffer, blocking ? CL_TRUE : CL_FALSE, 0, s == 0 ? size
			: s, hostBuffer, 0, 0, eventToSet ));
}
...
Kernel::Kernel(char* file, char* entryPoint)
{
	...
	error = clBuildProgram(program, 0, NULL, "-cl-fast-relaxed-math -cl-mad-enable -cl-single-precision-constant", NULL,
	                       NULL);
	...
	if (error == CL_SUCCESS)
	{
		...
		auto sizes = new size_t[devCount];
		sizes[0] = 0;
		size_t received;
		CHECKCL(
			clGetProgramInfo( program, CL_PROGRAM_BINARY_SIZES /* wrong data... */, devCount * sizeof( size_t ), sizes,
				&received ));
		auto binaries = new char*[devCount];
	...
	else
	{
		...
		if (errorString)
		{
			...
			if (!lns) FatalError(log + errorPos);
			else
			{
				lns += 2;
...
bool Kernel::InitCL()
{
	...
	for (uint i = 0; i < devCount; i++)
	{
		...
		if (extensionSize > 0)
		{
			auto extensions = (char*)malloc(extensionSize);
			CHECKCL(
				error = clGetDeviceInfo( devices[i], CL_DEVICE_EXTENSIONS, extensionSize, extensions, &extensionSize ));
...
void Kernel::Run(const size_t count, const size_t localSize, cl_event* eventToWaitFor, cl_event* eventToSet)
{
	...
	if (acqBuffer)
	{
		...
		CHECKCL(
			error = clEnqueueNDRangeKernel( queue, kernel, 1, 0, &count, localSize == 0 ? 0 : &localSize, eventToWaitFor
				? 1 : 0, eventToWaitFor, eventToSet ));
...
void Kernel::Run2D(const int2 count, const int2 lsize, cl_event* eventToWaitFor, cl_event* eventToSet)
{
	...
	if (acqBuffer)
	{
		...
		CHECKCL(error = clEnqueueAcquireGLObjects( queue, 1, acqBuffer->GetDevicePtr(), 0, 0, 0 ));
		CHECKCL(
			error = clEnqueueNDRangeKernel( queue, kernel, 2, 0, workSize, localSize, eventToWaitFor ? 1 : 0,
				eventToWaitFor, eventToSet ));
		CHECKCL(error = clEnqueueReleaseGLObjects( queue, 1, acqBuffer->GetDevicePtr(), 0, 0, 0 ));
	}
	else
	{
